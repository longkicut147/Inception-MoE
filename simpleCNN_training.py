'''
This script pretrains the SimpleCNN model on the CIFAR-10 dataset (5 train batches) and saves the weights for three different pretraining datasets.
'''
...
for train_dataset, val_dataset, dataset_name in datasets:
    ...
    for epoch in range(num_epochs):
        ...
        if val_loss < best_val_loss:
            ...
            early_stop_counter = 0
            torch.save(model.state_dict(), f"SimpleCNN_weights_{dataset_name}.pth")
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(f'SimpleCNN_loss_{dataset_name}.png', bbox_inches='tight')
    plt.show()
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(f'SimpleCNN_accuracy_{dataset_name}.png', bbox_inches='tight')
    plt.show()
